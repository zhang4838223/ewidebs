 ...
 */
package net.ewide.platform.common.persistence;
...
import net.ewide.platform.common.utils.CookieUtils;
...
public class Page<T> {
	...
	private List<T> rows = new ArrayList<T>(); // 符合bootstrap table的前端接受的分页参数名称

	private long total; // 符合bootstrap table的前端接受的分页参数名称
	...
	/**
	 * 结合bootstrap table服务端分页构造方法
	 * 
	 * @param request
	 *            传递 repage 参数，来记住页码
	 * @param response
	 *            用于设置 Cookie，记住页码
	 * @param sidePagination
	 *            需要服务端分页，随便传字符串         
	 */
	public Page(HttpServletRequest request, HttpServletResponse response, String sidePagination){
		this(request, response, -2,sidePagination);
	}
	/**
	 * 构造方法
	 * 
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize) {
		...
		// String no = request.getParameter("pageNo");    根据bootstrap table特定参数调整
		int offset = Integer.parseInt(request.getParameter("offset"));
		String size = request.getParameter("limit");
		if (offset != 0) {// 获取页数
			offset = offset / Integer.parseInt(size);
			  }
		offset += 1;
		String no = String.valueOf(offset);
		...
		// String size = request.getParameter("pageSize");  根据bootstrap table特定参数调整
	 ...
	 *            默认分页大小，如果传递 -1 则为不分页，返回所有数据
	 */
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize , String sidePagination) {
		// 设置页码参数（传递repage参数，来记住页码）
		// String no = request.getParameter("pageNo");    根据bootstrap table特定参数调整
		int offset = 0;
