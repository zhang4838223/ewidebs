package net.ewide.platform.common.utils;
...
public class UploadUtils {
	...
	public UploadUtils() {
		...
		PropertiesLoader loader = new PropertiesLoader("ewidebs.properties");
		maxSize=Long.parseLong(loader.getProperty("web.maxUploadSize", "10485760"));
	}
	 ...
	 * 
	 * @param request
	 * @return infos info[0] 验证文件域返回错误信息 info[1] 上传文件错误信息 info[2] savePath info[3] saveUrl info[4] fileUrl info[5]原始文件名
	 */
	...
	private String validateFields(HttpServletRequest request) {
		...
		// 文件保存目录URL
//		saveUrl = request.getContextPath() + "/" + basePath + "/"; // 带项目名称
		saveUrl = basePath + "/";
...
//	@SuppressWarnings("unchecked")
	...
	private String saveFile(FileItem item) {
		...
		if(Arrays.<String> asList(extMap.get("images").split(",")).contains(fileExt)){
			...
			dirName="flashs";
			saveUrl=saveUrl.replace("images", "flashs");
			savePath=savePath.replace("images", "flashs");
		}else if(Arrays.<String> asList(extMap.get("medias").split(",")).contains(fileExt)){
			dirName="medias";
			saveUrl=saveUrl.replace("images", "medias");
			savePath=savePath.replace("images", "medias");
		}else if(Arrays.<String> asList(extMap.get("files").split(",")).contains(fileExt)){
			dirName="files";
			saveUrl=saveUrl.replace("images", "files");
			savePath=savePath.replace("images", "files");
		}else{
