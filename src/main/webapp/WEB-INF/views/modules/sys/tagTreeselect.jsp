	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			$.get("${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&t="
				...
				selectCheckNode();
				var iframeLength = $(window.parent.document).find("iframe").length;
				for(var aa=0;aa<iframeLength;aa++){
					if(typeof $(window.parent.document).find("iframe")[aa].contentWindow.treeSelectLoaded == 'function'){
						$(window.parent.document).find("iframe")[aa].contentWindow.treeSelectLoaded(tree);
					}
				}
			});
			key = $("#key");
